# coding=utf-8
# !/usr/bin/python
# 蜜桃视频 T3 爬虫（集成涩库本地播放 + 蜜桃在线 + 智能请求头）
import sys
sys.path.append('..')
...
class SikuEngine:
    ...
    def _legacy_fetch_video_list(self, conn, db_key, table_name, mapping, filter_field, category_val, pg, limit, offset):
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if not pic: pic = self.MOVIE_ICON
                vod_list.append({
    ...
    def detailContent(self, ids):
        mid = ids[0]
        if mid.startswith("siku:"): mid = mid[5:]
        if '#ID#' in mid:
            parts = mid.split('#ID#')
    ...
    def playerContent(self, flag, id, vipFlags=None):
        ...
        headers = {
            "User-Agent": random.choice([
                "Dalvik/2.1.0 (Linux; U; Android 9; MIbox PRO Build/PI)",
